...
roll_animation() {
    ...
    if [ "$roll" -lt 1 ]; then
        ...
        count_semirare=$((count_semirare + 1))
        draw_box 0 "$RED" "$S_SEMIRARE"
        printf "\033[K%b\n" "${RED}SEMIRARE -- 15% chance${RESET}"

    elif [ "$roll" -lt 40 ]; then
        count_uncommon=$((count_uncommon + 1))
        draw_box 0 "$GREEN" "$S_UNCOMMON"
        printf "\033[K%b\n" "${GREEN}UNCOMMON -- 15% chance${RESET}"
    else
...
while true; do
    ...
    if [ "$key" = "q" ] || [ "$key" = "Q" ]; then
        ...
        printf "total rolls: %s  (common %s, uncommon %s, semirare %s, rare %s, legendary %s)\n" \
            "$total_rolls" "$count_common" "$count_uncommon" "$count_semirare" "$count_rare" "$count_legendary"
        break
    fi
